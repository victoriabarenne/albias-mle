...
class SnelsonDataset(ActiveLearningDataset):
    def __init__(self, path_to_data, random_state):
        ...
        self.idx_train= np.random.choice(np.arange(len(y)), int(0.8*len(y)), replace= False)
        self.idx_test= np.where(np.isin(np.arange(len(y)), self.idx_train, )==False)[0]
        if random_state is not None:
            np.random.set_state(state)

        self.dataset= TensorDataset(x, y)
        self.all = WeightedSubset(self.dataset, self.idx_train)
        self.n_points= len(self.all)
        self.available = WeightedSubset(self.dataset, self.idx_train)
        self.train = WeightedSubset(self.dataset, np.array([], dtype=int))
        self.test = WeightedSubset(self.dataset, self.idx_test)


        self.labeled= np.zeros(len(self.available), dtype=int)
        self.queries = np.array([], dtype=int)
        self.acq_prob = torch.tensor([])
    ...
    def get_loader(self, type: str, batch_size, model_arch, laplace, variational_samples= None):
        if type=="train":
            ...
            initial_dataset= self.test
        elif type == "all":
            initial_dataset = self.all

        if not model_arch=="mlp":
            AssertionError("Only mlp model architecture is valid for this dataset")

        if laplace:
            dataset= Subset(initial_dataset, np.arange(len(initial_dataset)), unsqueeze=False)
        else:
            dataset= initial_dataset
        dataset.targets= dataset.targets.float()
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
        return loader
